...
function imprimirSeries(series : Serie[]):void{
    for (let i = 0; i < series.length; i++) {
        mostrarDatosSeries(series[i]);
    }
}

function darPromedioSeasons(series : Serie[]):number{
    let promedioSeasons = 0;
    for (let i = 0; i < series.length; i++) {
        promedioSeasons += series[i].seasons;
    }

    return promedioSeasons/series.length;
}
